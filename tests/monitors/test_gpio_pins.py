...
def test_fmt_gpio_fixed_pin_chars():
    ...
    data = {"gpio_pins": gpio_pins.read()["gpio_pins"]}
    row1, row2 = _fmt_gpio(data)
    # Physical pin 1 (index 0, odd → row1 pos 0) → "+"
    assert row1[0] == "+"
    # Physical pin 2 (index 1, even → row2 pos 0) → "^"
    assert row2[0] == "^"
    # Physical pin 6 (index 5, even → row2 pos 2) → "_"
    assert row2[2] == "_"
...
def test_fmt_gpio_gpio_chars(fake_sysfs):
    ...
    data = {"gpio_pins": gpio_pins.read()["gpio_pins"]}
    row1, row2 = _fmt_gpio(data)
    # Physical pin 7 (index 6, odd → row1 pos 3) → chr(0) output high
    assert row1[3] == chr(0)
