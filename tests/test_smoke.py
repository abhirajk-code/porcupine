...
def test_all_monitor_modules_importable():
    from porcupine.monitors import power, cpu_mem, temperature, network
    for mod in (power, cpu_mem, temperature, network):
        assert callable(mod.read)


def test_all_interface_classes_importable():
    from porcupine.interfaces.lcd import LCD
    from porcupine.interfaces.button import Button, MenuFSM
    from porcupine.interfaces.buzzer import Buzzer, AlertChecker
    for cls in (LCD, Button, MenuFSM, Buzzer, AlertChecker):
        assert callable(cls)
...
def test_help_lists_monitor_flags():
    ...
    for flag in ("--power", "--cpu", "--temp", "--net"):
...
def test_build_screens_end_to_end(tmp_path):
    ...
    args = parse_args(["--no-temp", "--no-net"], config_path=str(tmp_path / "none.conf"))
    data = {
...
def test_read_all_disabled_monitors_returns_empty(tmp_path):
    ...
    args = parse_args(
        ["--no-power", "--no-cpu", "--no-temp", "--no-net"],
