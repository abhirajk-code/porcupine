...
def test_temp_alert_fires_short_beep_above_threshold(checker):
    ...
    assert pin_log(bz).count(True) == 1  # 1 short beep
...
def test_temp_alert_fires_every_cycle(checker):
    ...
    _wait_for_beep(bz)
    time.sleep(0.05)
    assert pin_log(bz).count(True) == 3  # one beep per check


def test_cpu_alert_fires_immediately_no_sustained_delay(checker):
    bz, ac = checker
    with patch("porcupine.interfaces.buzzer.time.sleep"):
...
def test_mem_alert_fires_short_beep_above_threshold(checker):
    bz, ac = checker
    with patch("porcupine.interfaces.buzzer.time.sleep"):
...
def test_bat_alert_fires_long_beep_below_threshold(checker):
    ...
    assert _wait_for_beep(bz)
    assert pin_log(bz).count(True) == 1
    assert sleep_calls[0] == pytest.approx(0.6)  # 600 ms long beep
